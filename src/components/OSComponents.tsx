  ...
  return (
    <motion.div
      ...
      style={{ 
        ...
        x: (window.innerWidth / 2) - ((win.defaultWidth || 800) / 2) + (Math.random() * 30 - 15), 
        y: 100 + (Math.random() * 30 - 15)
      }}
      className="absolute flex flex-col rounded-xl overflow-hidden shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5),0_0_0_1px_rgba(255,255,255,0.1)] bg-[#1e1e1e]/90 backdrop-blur-2xl border border-white/5 ring-1 ring-black/20"
    >
      {/* Window Title Bar */}
      <div 
      ...
      >
        ...
        <div className="text-xs font-medium text-white/50 select-none tracking-wide">{win.title}</div>
...
export const Dock: React.FC<DockProps> = ({ windows, onOpen }) => {
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 px-4 pb-4 pt-5 rounded-[2rem] bg-white/10 backdrop-blur-3xl border border-white/10 shadow-[0_20px_40px_rgba(0,0,0,0.3)] flex items-end gap-4 z-[9999] hover:scale-[1.02] transition-transform duration-500">
      {windows.map((win) => (
    ...
    return (
        <div className="relative flex flex-col items-center justify-end h-full gap-2">
            ...
            <motion.button
                ...
                className="w-14 h-14 rounded-2xl shadow-[0_10px_20px_rgba(0,0,0,0.4)] relative overflow-hidden group bg-gradient-to-b from-white/10 to-white/5 border border-white/10 ring-1 ring-black/20"
            >
                <div className="absolute inset-0 bg-white/20 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none mix-blend-overlay"></div>
                {window.icon}
            </motion.button>
...
export const Menubar: React.FC = () => {
    ...
    return (
        <div className="fixed top-0 left-0 right-0 h-8 bg-black/20 backdrop-blur-md flex items-center justify-between px-5 z-[9999] text-[13px] font-medium text-white shadow-sm">
            <div className="flex items-center gap-6">
                <Apple size={16} fill="white" className="drop-shadow-sm hover:opacity-80 transition-opacity cursor-pointer" />
                <span className="hidden md:inline font-bold drop-shadow-sm">CloudOS</span>
                <span className="hidden md:inline drop-shadow-sm opacity-80 hover:opacity-100 cursor-default transition-opacity">File</span>
                <span className="hidden md:inline drop-shadow-sm opacity-80 hover:opacity-100 cursor-default transition-opacity">Edit</span>
                <span className="hidden md:inline drop-shadow-sm opacity-80 hover:opacity-100 cursor-default transition-opacity">View</span>
                <span className="hidden md:inline drop-shadow-sm opacity-80 hover:opacity-100 cursor-default transition-opacity">Go</span>
                <span className="hidden md:inline drop-shadow-sm opacity-80 hover:opacity-100 cursor-default transition-opacity">Window</span>
                <span className="hidden md:inline drop-shadow-sm opacity-80 hover:opacity-100 cursor-default transition-opacity">Help</span>
            </div>
