    ...
    >
      <div className="container mx-auto max-w-6xl flex items-center justify-between gap-4 md:gap-8">
        ...
        <div className="flex items-center gap-4 min-w-0 w-1/3 md:w-1/4 cursor-pointer group" onClick={onOpenDetail}>
          <div className={`relative w-12 h-12 md:w-14 md:h-14 rounded overflow-hidden shrink-0 border border-white/10 group-hover:border-hot-pink/50 transition-colors ${isPlaying ? 'shadow-[0_0_20px_rgba(255,0,128,0.5)] border-hot-pink/30' : ''}`}>
             <img 
                src={track.coverUrl} 
                alt={track.title} 
                className={`w-full h-full object-cover transition-all duration-500 ${isPlaying ? 'animate-pulse opacity-90 scale-105' : 'opacity-100 scale-100'}`} 
             />
